...
impl Chip9 {
    ...
    pub fn execute(&mut self) -> Result<(), Chip9Error> {
        ...
        match opcode {
            OpCode::NoOp => (),
            OpCode::ClearScreen => self.cleared_screen(),
            ...
            OpCode::SkipKeyPressed(x) => self.skip_key_pressed(x),
            OpCode::SkipKeyNotPressed(x) => self.skip_key_not_pressed(x),
    ...
    fn draw(&mut self, vx: Nib, vy: Nib, height: Nib) {
    ...
    fn skip_key_pressed(&mut self, vx: Nib) {
        if self.keyboard.is_key_pressed(self.regs[vx]) {
            self.pc += 2;
        }
    }
